...
public class ArchiveService {
    ...
    public ArchiveResponse getArchiveMetadata() {
        List<AnneeScolaire> academicYears = anneeScolaireRepository.findAll()
                .stream()
                .filter(year -> !year.getCourante())
                .collect(Collectors.toList());
                
        List<Filiere> filieres = filiereRepository.findAll();

        return ArchiveResponse.builder()
                .academicYears(academicYears.stream()
    ...
    public BinomeManagementResponse getBinomeManagementDataByYear(Long filiereId, Long anneeScolaireId) {
        ...
        if (filiereId != null) {
            ...
            List<SujetDTO> availableSujets = sujetRepository
                    .findByFiliereAndAnneeScolaire(filiere, year)
                    .stream()
    ...
    public NoteManagementResponse getNotesByFiliereAndYear(Long filiereId, Long anneeScolaireId) {
        ...
        for (Etudiant etudiant : students) {
            ...
            // Find the student's binôme for specified year
            ...
            // For simplicity, adding default values for soutenance and encadrant
    ...
    public YearDataExportDTO exportYearData(Long anneeScolaireId) {
        AnneeScolaire year = findAnneeScolaireById(anneeScolaireId);
        
        // Collect all data for the year
        CompteManagementResponse comptes = getComptesByRoleAndYear(null, anneeScolaireId);
        BinomeManagementResponse binomes = getBinomeManagementDataByYear(null, anneeScolaireId);
        SujetManagementResponse sujets = getSujetsByFiliereAndYear(null, anneeScolaireId);
        List<SoutenanceDTO> soutenances = getSoutenancesByFiliereAndYear(null, anneeScolaireId);
        NoteManagementResponse notes = getNotesByFiliereAndYear(null, anneeScolaireId);
        
        return YearDataExportDTO.builder()
                .academicYear(year.getAnnee())
                .comptes(comptes.getComptes())
                .binomes(binomes.getBinomes())
                .sujets(sujets.getSujets())
                .soutenances(soutenances)
                .notes(notes.getNotes())
                .filieres(getAllFilieres())
                .pourcentages(notes.getPourcentages())
                .build();
    }
    ...
    // Mapping methods (reusing from other services)
