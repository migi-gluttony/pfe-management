<template>
    <div class="jury-grading">
      ...
      <div class="content-container">
                ...
                <template #body="slotProps">
                  <div class="binome-students">
                    <div class="student">
                      ...
                      <span>{{ slotProps.data.binome?.etudiant1?.prenom }} {{ slotProps.data.binome?.etudiant1?.nom }}</span>
                    </div>
                    <div v-if="slotProps.data.binome?.etudiant2" class="student">
                      <i class="pi pi-user"></i>
                      <span>{{ slotProps.data.binome?.etudiant2?.prenom }} {{ slotProps.data.binome?.etudiant2?.nom }}</span>
                    </div>
                  </div>
                </template>
              </Column>
              ...
              <Column header="Status">
                <template #body="slotProps">
                  <Tag 
                    ...
                    :severity="getEvaluationStatusSeverity(slotProps.data)" 
                  />
                </template>
              </Column>
            </DataTable>
          ...
          <template #content>
            <div class="eval-form">
              ...
              <div class="form-field">
                <label for="presentationScore">Qualité de la Présentation (0-20)</label>
                <div class="score-input">
                  <Rating 
                    v-model="evaluationForm.presentationScoreRating" 
                    :cancel="false" 
                    :stars="4"
                ...
                <small>Évaluez la qualité et la clarté de la présentation</small>
              </div>
  
              <!-- Q&A evaluation -->
              <div class="form-field">
                <label for="qaScore">Réponses aux Questions (0-20)</label>
                <div class="score-input">
                  <Rating 
                ...
                <small>Évaluez la qualité des réponses aux questions</small>
              </div>
  
              <!-- Time management evaluation -->
              <div class="form-field">
                <label for="timeManagementScore">Gestion du Temps (0-20)</label>
                <div class="score-input">
                  <Rating 
                    v-model="evaluationForm.timeManagementScoreRating" 
                    :cancel="false" 
                    :stars="4"
        ...
        <Card v-else-if="selectedSoutenance" class="evaluation-card">
          <template #title>
            <div class="card-title">
              <i class="pi pi-users" style="font-size: 1.5rem; margin-right: 0.5rem;"></i>
              Sélectionner un Étudiant
            </div>
          </template>
          <template #content>
            <div class="students-selection">
              <p class="instructions">Veuillez sélectionner un étudiant à évaluer :</p>
              
              <div class="student-option" @click="selectStudent(selectedSoutenance.binome.etudiant1)">
                <Avatar icon="pi pi-user" size="large" />
                <div class="student-info">
                  <h3>{{ selectedSoutenance.binome.etudiant1.prenom }} {{ selectedSoutenance.binome.etudiant1.nom }}</h3>
                  <p>{{ getStudentEvaluationStatus(selectedSoutenance.binome.etudiant1) }}</p>
                </div>
                <Button icon="pi pi-arrow-right" class="p-button-rounded p-button-outlined" />
              </div>
              
              <div v-if="selectedSoutenance.binome.etudiant2" class="student-option" @click="selectStudent(selectedSoutenance.binome.etudiant2)">
                <Avatar icon="pi pi-user" size="large" />
                <div class="student-info">
                  <h3>{{ selectedSoutenance.binome.etudiant2.prenom }} {{ selectedSoutenance.binome.etudiant2.nom }}</h3>
                  <p>{{ getStudentEvaluationStatus(selectedSoutenance.binome.etudiant2) }}</p>
                </div>
                <Button icon="pi pi-arrow-right" class="p-button-rounded p-button-outlined" />
              </div>
            </div>
        ...
        <Card v-else class="evaluation-card info-card">
          <template #content>
            <div class="info-message">
              ...
              <p>Choisissez une soutenance dans la liste pour commencer votre évaluation</p>
            </div>
          </template>
        </Card>
      </div>
  ...
  const filteredSoutenancesValue = computed(() => {
    ...
    if (selectedDate.value) {
      const formattedSelectedDate = new Date(selectedDate.value).toISOString().split('T')[0];
    ...
    if (searchQuery.value) {
      ...
      filtered = filtered.filter(soutenance => {
        const etudiant1Name = `${soutenance.binome?.etudiant1?.prenom} ${soutenance.binome?.etudiant1?.nom}`.toLowerCase();
        ...
        return etudiant1Name.includes(query) || 
  ...
  function filterSoutenancesByDate() {
    // Date filter is handled by filteredSoutenances computed property
    resetSelection();
  }
  ...
  function getStudentEvaluationStatus(student) {
    // In a real application, this would check if an evaluation exists already
    // For now, let's use a placeholder logic
    if (selectedSoutenance.value?.evaluations?.find(e => e.etudiantId === student.id)) {
  ...
  function calculatePresentationScore() {
    evaluationForm.value.presentationScore = Math.round(evaluationForm.value.presentationScoreRating * 5);
  }
  
  function calculateQAScore() {
    evaluationForm.value.qaScore = Math.round(evaluationForm.value.qaScoreRating * 5);
  }
  
  function calculateTimeScore() {
    evaluationForm.value.timeManagementScore = Math.round(evaluationForm.value.timeManagementScoreRating * 5);
  }
  ...
  async function submitEvaluation() {
    ...
    try {
      const response = await ApiService.post('/grading/jury/soutenance-evaluation', payload);
      
      // Update soutenance in the list to mark as evaluated
      const index = soutenances.value.findIndex(s => s.id === selectedSoutenance.value.id);
      if (index !== -1) {
  ...
  function validateEvaluationForm() {
    if (!evaluationForm.value.soutenanceId || !evaluationForm.value.etudiantId) {
      showValidationError('Erreur de sélection de la soutenance ou de l\'étudiant');
      return false;
    }
    
    if (
      evaluationForm.value.presentationScore < 0 || evaluationForm.value.presentationScore > 20 ||
      evaluationForm.value.qaScore < 0 || evaluationForm.value.qaScore > 20 ||
      evaluationForm.value.timeManagementScore < 0 || evaluationForm.value.timeManagementScore > 20
    ) {
  ...
  .soutenances-card, .evaluation-card {
    width: 100%;
  }
