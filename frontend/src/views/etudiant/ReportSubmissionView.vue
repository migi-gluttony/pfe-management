<template>
    <div class="report-submission-view">
        ...
        <div class="view-container">
            ...
            <div class="header-section">
                <div class="title-filter-group">
                    ...
                    <p class="page-subtitle">Soumettez votre rapport final pour votre projet de fin d'études</p>
                </div>
            </div>
                ...
                <template #content>
                    ...
                    <div v-else>
                        <div class="soutenance-status" v-if="soutenanceInfo">
                            ...
                            <div class="status-content">
                                <div class="status-row">
                                    ...
                                    <span class="value">{{ formatDate(soutenanceInfo.date) }}</span>
                                </div>
                                <div class="status-row">
                                    <span class="label">Heure:</span>
                                    <span class="value">{{ formatTime(soutenanceInfo.heure) }}</span>
                                </div>
                                <div class="status-row">
                                    <span class="label">Salle:</span>
                                    <span class="value">{{ soutenanceInfo.salle.nom }}</span>
                                </div>
                                <div class="status-row">
                                    <span class="label">Statut du dépôt:</span>
                                    <Tag 
                                        :severity="existingReport ? 'success' : 'warning'" 
                                        :value="existingReport ? 'Rapport déposé' : 'Rapport non déposé'"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="!soutenanceInfo" class="no-soutenance">
                            ...
                            <p>Vous n'avez pas encore de soutenance planifiée. Votre chef de département doit planifier votre soutenance avant que vous puissiez déposer votre rapport.</p>
                        </div>
                        
                        <div v-else-if="!canUpload" class="upload-disabled">
                            <i class="pi pi-lock"></i>
                            <h3>Dépôt de rapport bloqué</h3>
                            <p>Vous ne pouvez plus modifier votre rapport car votre soutenance est prévue dans moins de 3 jours. Contactez votre encadrant si vous avez besoin de faire des modifications.</p>
                            
                            <div v-if="existingReport" class="existing-report-details">
                                <h4>Votre rapport déposé</h4>
                                <div class="report-info">
                                    <div class="report-row">
                                        <span class="label">Titre:</span>
                                        <span class="value">{{ existingReport.titre }}</span>
                                    </div>
                                    <div class="report-row">
                                        <span class="label">Date de dépôt:</span>
                                        <span class="value">{{ formatDateTime(existingReport.dateSoumission) }}</span>
                                    </div>
                                    <Button 
                                        label="Télécharger" 
                                        icon="pi pi-download" 
                        ...
                        <div v-else class="upload-section">
                            ...
                            <div v-else class="no-report">
                                ...
                                <p>Vous n'avez pas encore déposé votre rapport final. Veuillez le déposer au moins 3 jours avant la date de votre soutenance.</p>
                                <Button 
                    ...
                    <div class="guidelines-content">
                        <div class="guideline-item">
                            ...
                            <div class="guideline-text">
                                ...
                                <p>Le rapport doit être au format PDF ou Word (DOC/DOCX). La taille maximum du fichier est de 20 Mo.</p>
                            </div>
                        </div>
                        ...
                        <div class="guideline-item">
                            ...
                            <div class="guideline-text">
                                ...
                                <p>Vous devez déposer votre rapport au moins 3 jours avant la date de votre soutenance. Après ce délai, vous ne pourrez plus modifier votre rapport.</p>
                            </div>
                        </div>
                        ...
                        <div class="guideline-item">
                            ...
                            <div class="guideline-text">
                                ...
                                <p>Votre rapport doit inclure : une introduction, une présentation du problème, la méthodologie utilisée, les résultats obtenus, les limitations et perspectives, et une conclusion.</p>
                            </div>
                        </div>
                    </div>
                </template>
        ...
        <Dialog 
            ...
            :modal="true" 
            :header="isReplacing ? 'Remplacer le Rapport' : 'Déposer le Rapport'" 
            :style="{ width: '50vw' }"
            :closable="false"
        >
            <div class="p-fluid upload-form">
                <div class="field">
                    <label for="reportTitle">Titre du rapport<span class="required-marker">*</span></label>
                    <InputText 
                        id="reportTitle" 
                ...
                <div class="field">
                    <label>Fichier du rapport<span class="required-marker">*</span></label>
                    <FileUpload
                        ref="fileUploadRef"
                        :multiple="false"
                        accept=".pdf,.doc,.docx"
                    ...
                    >
                        <template #empty>
                            <p>Glissez et déposez votre rapport ici ou cliquez pour parcourir.</p>
                            <p class="file-types">Formats supportés: PDF, DOC, DOCX</p>
                        </template>
                    </FileUpload>
                ...
                <div class="confirmation-message">
                    ...
                    <p>En déposant ce rapport, vous confirmez qu'il s'agit de votre version finale pour la soutenance. Vous pourrez le remplacer jusqu'à 3 jours avant la date de soutenance.</p>
                </div>
            </div>
...
export default {
    ...
    setup() {
        ...
        const checkUploadPermission = async () => {
            try {
                const response = await ApiService.get('/etudiant/rapport/can-upload');
                canUpload.value = response.canUpload;
            } catch (error) {
                handleError(error, 'Erreur lors de la vérification des permissions');
        ...
        const submitReport = async () => {
            ...
            try {
                ...
                toast.add({
                    severity: 'success',
                    summary: isReplacing.value ? 'Rapport remplacé' : 'Rapport déposé',
                ...
                handleError(error, isReplacing.value 
        ...
        const confirmDeleteReport = () => {
            confirm.require({
                message: 'Êtes-vous sûr de vouloir supprimer votre rapport ? Vous devrez en déposer un nouveau avant votre soutenance.',
        ...
        const downloadReport = (report) => {
            ...
            window.open(`/api/etudiant/rapport/download/${report.id}`, '_blank');
        };
...
/* No Soutenance state */
.no-soutenance, .upload-disabled, .no-report {
    display: flex;
    flex-direction: column;
    align-items: center;
    text-align: center;
...
.no-soutenance i, .upload-disabled i, .no-report i {
    font-size: 3rem;
    color: var(--text-color-secondary);
    margin-bottom: 1rem;
}

.no-soutenance h3, .upload-disabled h3, .no-report h3 {
    margin: 0 0 0.5rem;
    color: var(--text-color);
}

.no-soutenance p, .upload-disabled p, .no-report p {
    margin: 0;
    color: var(--text-color-secondary);
    max-width: 600px;
}
